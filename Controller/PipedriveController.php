...
class PipedriveController extends CommonController
{
	...
	const THRASHHOLD_POINTS = 3; //if points greater than this passon else reject
	const PIPEDRIVEKEY = "93f56338d3088b38366d611e881b373e0708f189";
	const PIPEDRIVE_OWNER_ID = 1763084; //your your id in pipedrive which becomes owner of person, org, activity
	/**
	...
	*/
	public function personAction(){
		...
		$name = $firstName." ".$lastName; //join firstName and lastName to form name in Pipedrive format(fname lname)
		...
		//custom field values from Mautic
		$productionType = $data['mautic.lead_points_change'][0]['lead']['fields']['professional']['production_type']['value']; //production type
		$show = $data['mautic.lead_points_change'][0]['lead']['fields']['professional']['f_show']['value']; //show name
		$other = $data['mautic.lead_points_change'][0]['lead']['fields']['professional']['other_show']['value']; //other show name
		$openingDate = $data['mautic.lead_points_change'][0]['lead']['fields']['professional']['opening_date']['value']; //opening date
		$closingDate = $data['mautic.lead_points_change'][0]['lead']['fields']['professional']['closing_date']['value']; //closing date
		$estSeatsSold = $data['mautic.lead_points_change'][0]['lead']['fields']['professional']['estimated_seats_sold_per']['value']; //estimated seats sold per performance
		$avgTicketPrice = $data['mautic.lead_points_change'][0]['lead']['fields']['professional']['average_ticket_price']['value']; //average ticket price
		$currency = $data['mautic.lead_points_change'][0]['lead']['fields']['professional']['currency']['value']; //currency
		$note = $data['mautic.lead_points_change'][0]['lead']['fields']['professional']['additional_information']['value']; //additional information
		...
		if(isset($points) && $points > self::THRASHHOLD_POINTS && (isset($email))){ //check thrashhold points and email existence
			...
			$person['email'] = $email;
			$person['57cfb485ebb77e2d7e3135f0631e0a9327654aea'] = $openingDate;
			$person['57cfb485ebb77e2d7e3135f0631e0a9327654aea_until'] = $closingDate;
